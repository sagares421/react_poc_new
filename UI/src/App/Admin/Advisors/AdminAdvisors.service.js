...
function getAll() {
    ...
    return fetch(`${config.apiUrl}/user/advisor`, requestOptions).then(handleResponse);
}

function getById(id) {
    const requestOptions = { method: 'GET', headers: authHeader() };
    return fetch(`${config.apiUrl}/user/advisor/${id}`, requestOptions).then(handleResponse);
}
